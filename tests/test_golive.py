...
def _valid_runcards(tmp_path):
    d = tmp_path / "runcards"
    d.mkdir(parents=True, exist_ok=True)
    (d / "valid.yaml").write_text(
        _yaml.safe_dump({"validation": {"permutation_p": 0.01}, "verdict": "edge"}),
        encoding="utf-8",
    )
    return str(d)


def _ladder(tmp_path, **kwargs):
    return GoLiveLadder(state_path=str(tmp_path / "gl.json"),
                        runcards_dir=_valid_runcards(tmp_path), **kwargs)
...
def test_ladder_locks_live_until_all_gates_signed(tmp_path):
    ...
    assert _ladder(tmp_path).live_allowed() is True
            ...
            def execute(q, *a):
                class R:
                    def fetchone(self):
                        ...
                        return {"a": a0, "b": b0}
                return R()
    lad = _ladder(tmp_path, journal=_J())
...
def test_testnet_probe_reports_place_failure(tmp_path):
    ...
    res = run_testnet_probe(_Broker(), _ladder(tmp_path))
