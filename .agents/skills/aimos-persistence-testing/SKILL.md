...
- Python 3.11.11 via pyenv and the editable package with extras that include `serve`,
  `runtime`, `data`, and `saas`:
  ```bash
  export PATH="$HOME/.pyenv/bin:$PATH"
  eval "$(pyenv init -)"
  pyenv local 3.11.11
  python -m pip install -e '.[runtime,serve,data,saas]'
  ```
- `serve.py` imports `aimos.saas.auth_service` which in turn imports `bcrypt`, so the
  `[saas]` extra (or `bcrypt`) is required even when SaaS mode is disabled.
